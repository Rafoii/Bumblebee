  ...
  <Layout>
    ...
    >
      <v-card>
        ...
        <v-card-text
          >Close "{{ $store.state.datasets[confirmDelete].name }}"?</v-card-text
        >
        <v-card-actions>
          <div class="flex-grow-1" />
          <v-btn color="primary" text @click="confirmDelete = -1">Cancel</v-btn>
          <v-btn color="primary" text @click="deleteTab(confirmDelete)"
            >Accept</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    <v-layout
      row
      wrap
      class="elevation-0 d-flex flex-column align-top justify-start"
    >
      ...
      <template v-else>
          ...
          <div class="center-screen-inside black--text">
            ...
            <v-card class="elevation-0">
              <v-card-text class="title mb-2"
                >Now send info from your notebook. Try something
                like:</v-card-text
              >
              <v-card-text class="subtitle text-code">
                ...
                <br />op = Optimus(comm= <span class="keyword">True</span>)
                <br />
                <br />
                ...
                <br />df.ext.send( <span class="string">"Meteorite"</span>)
              </v-card-text>
            </v-card>
          </div>
          <v-icon class="back-btn" large color="black" @click="stopClient"
            >arrow_back</v-icon
          >
        </template>
        <div v-else class="bb-container">
          <v-tabs
            ...
            v-model="tab"
            :class="{
              'tabs-disabled': $store.state.everyPreviewCode[tab] || isOperating
            }"
            class="bb-tabs px-6"
          ...
          <div class="bb-content">
            <TableBar
              ...
              :total="
                currentDataset.summary ? +currentDataset.summary.rows_count : 1
              "
            />
          </div>
            ...
            <v-layout class="px-4" row justify-space-between>
              ...
              >
                <template v-if="currentDataset.summary.total_count_dtypes"
                  >{{
                    currentDataset.summary.total_count_dtypes | formatNumberInt
                  }}
                  Data types &emsp;</template
                >
                <template v-if="sampleSize"
                  >{{ sampleSize | formatNumberInt }} of</template
                >
                <template v-if="currentDataset.summary.rows_count"
                  >{{
                    currentDataset.summary.rows_count | formatNumberInt
                  }}
                  Rows &emsp;</template
                >
                <template v-if="currentDataset.summary.cols_count"
                  >{{
                    currentDataset.summary.cols_count | formatNumberInt
                  }}
                  Columns</template
                >
              </span>
...
export default {
  ...
  methods: {
    ...
    async register() {
      try {
        if (
          this.confirmPassword !== this.createPassword &&
          !this.showPassword
        ) {
