...
export const arrayJoin = (array, separator = ', ', finalSeparator = ' and ') => {
  var outStr = ''
  if (array.length === 1) {
    outStr = array[0]
  } else if (array.length === 2) {
    outStr = array.join(finalSeparator)
  } else if (array.length > 2) {
    outStr = array.slice(0, -1).join(separator) + finalSeparator + array.slice(-1)
  }
  return outStr
}
...
export const newName = (name) => {
  ...
  if (matches) {
    ...
    const prefix = name.substr(0, matches.index)
...
export const reduceRanges = (rangesArray) => {
  return rangesArray.reduce((ranges, range) => {
    if (ranges.length === 0) {
      return [range]
    } else {
